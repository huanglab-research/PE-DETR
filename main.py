...
def get_args_parser():
    ...
    parser.add_argument('--use_physics', action='store_true', help='enable physics feature branch')
    parser.add_argument('--phys_channels', type=int, default=32)
    parser.add_argument('--use_optflow_proxy', action='store_true')
    parser.add_argument('--use_phys_query_init', action='store_true', help='use physics+visual cues to init queries')
    parser.add_argument('--use_ms_physics', action='store_true',
                        help='Use multi-scale physics heads + fusion (structure-only change, defaults to OFF for fair ablation)')
        ...
        if not args.eval and 'optimizer' in checkpoint and 'lr_scheduler' in checkpoint and 'epoch' in checkpoint:
            ...
            if not args.drop_lr_now:
                ...
                if args.override_resumed_lr_drop:
                    print(
                        'Warning: args.override_resumed_lr_drop is set to True, so args.lr_drop would override lr_drop in resumed lr_scheduler.')
