...
import torch
from .physics import LearnablePhysHead, PhysVisualFusion, QueryInitializer, PhysHeadWithOW
...
class PEDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, 
                    ...
                    use_physics=False,
                    phys_channels=32,
                    use_optflow_proxy=False,
                    use_phys_query_init=False,
                    use_ms_physics=False,
                    ):
    ...
    def forward(self, samples: NestedTensor, targets:List=None):
        ...
        hs, reference, hs_enc, ref_enc, init_box_proposal = self.transformer(srcs, masks, input_query_bbox, poss,input_query_label,attn_mask, **extra_kwargs)
...
@MODULE_BUILD_FUNCS.registe_with_name(module_name='pedetr')
def build_pedetr(args):
    ...
    model = PEDETR(
        ...
        use_physics=getattr(args,'use_physics', False),
        phys_channels=getattr(args,'phys_channels', 32),
        use_optflow_proxy=getattr(args,'use_optflow_proxy', False),
        use_phys_query_init=getattr(args,'use_phys_query_init', False),
        use_ms_physics=getattr(args,'use_ms_physics', False),
    )
    if args.masks:
